...
namespace BethanysPieShopTests.Fakes
{
	public class RepositoryFakes
	{
		public static IPieRepository GetFakePieRepository()
		{
			...
			var mockPieRepository = A.Fake<IPieRepository>();
			A.CallTo(() => mockPieRepository.AllPies).Returns(pies);
			A.CallTo(() => mockPieRepository.AllPies).Returns(pies);
			A.CallTo(() => mockPieRepository.PiesOfTheWeek).Returns(pies.Where(p => p.IsPieOfTheWeek));
			A.CallTo(() => mockPieRepository.GetPieById(A<int>.Ignored)).Returns(pies[0]);

			return mockPieRepository;
		}
		

		public static ICategoryRepository GetFakeCategoryRepository()
		{
			...
			var mockCategoryRepository = A.Fake<ICategoryRepository>();
			A.CallTo(() => mockCategoryRepository.AllCategories).Returns(categories);
